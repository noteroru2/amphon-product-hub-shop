...
const checks = [
  ...
  ['Store Worker deploy requires Cloudflare token but not Shopee credentials', workflow.includes('secrets.CLOUDFLARE_API_TOKEN') && workflow.includes('Missing repository secret CLOUDFLARE_API_TOKEN') && workflow.includes('Shopee direct API disabled')],
