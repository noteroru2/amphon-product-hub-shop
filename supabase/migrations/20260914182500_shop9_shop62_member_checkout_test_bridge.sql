-- AMPHON SHOP — SHOP-9 / SHOP-6.2 isolated provider E2E bridge
-- Generated 2026-09-14
--
-- SHOP-8.5 introduced member-required checkout after SHOP-6.2's provider harness.
-- The isolated provider E2E must exercise payment/fulfillment through the production
-- order engine without requiring a real customer/Auth account. This wrapper already
-- uses a transaction-local settings bypass while public checkout remains disabled.
-- Extend that bypass to member_checkout_required only inside the same transaction,
-- and restore the exact original value before returning.
--
-- Public /store/checkout and create_commerce_order() remain unchanged and continue
-- enforcing member_checkout_required=true in production.
...
begin
  ...
  update public.commerce_store_settings
     set purchase_enabled = true,
         ...
         shop62_acceptance_evidence = jsonb_build_object(
           ...
           'memberCheckout','BYPASSED_ISOLATED_PROVIDER_E2E'
         )
   where id = 1;
  ...
  update public.commerce_store_settings
     set purchase_enabled = original.purchase_enabled,
         member_checkout_required = original.member_checkout_required,
...
comment on function public.create_commerce_test_order(jsonb, text) is
'SHOP-6.2/SHOP-9 isolated provider E2E wrapper. Public checkout must remain disabled; AT-TST-* only; member checkout is bypassed transaction-locally and restored before return.';
