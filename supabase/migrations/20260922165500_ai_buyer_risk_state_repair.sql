-- P0 risk-state repair and replay semantics for legitimate pricing blockers.
...
begin
  ...
  enriched as (
    select *,
      case
        when bucket='A_PRICE_NOW'
          and source_state='HUMAN_REVIEW'
          and pricing_review_reason in (
            'MARKET_COMPARABLES_INSUFFICIENT',
            'MARKET_PRICE_DISPERSION_HIGH',
            'MARKET_PRICING_CONFIDENCE_LOW',
            'PRICE_BOOK_AMBIGUOUS'
          )
          then 'HUMAN_REVIEW'
...
with risk_cases(case_id,reason_code) as (
  values
    ('d1243151-ccf1-45cf-9d85-b7e241553673'::uuid,'DEVICE_NOT_BOOTING'),
    ('eb850976-587a-4230-a340-bd109eac5e10'::uuid,'MAJOR_DAMAGE'),
    ('b8196370-63f9-47e8-a765-fe7b3b2d0b33'::uuid,'MODEL_SPEC_CONFLICT'),
    ('86c959b7-e210-466d-839a-91ab02b3872a'::uuid,'DEVICE_NOT_BOOTING'),
    ('0ad32432-c646-4e28-a1ae-ee81b796ae4f'::uuid,'OWNERSHIP_OR_FINANCE_REVIEW'),
    ('d75a1c78-da8c-44ec-9aca-06adb627a74b'::uuid,'DEVICE_NOT_BOOTING'),
    ('dd18a3c6-c93c-4cae-a772-a464917ac3fe'::uuid,'MULTIPLE_DEVICES_AMBIGUOUS')
)
update public.ai_buyer_valuation_cases c
set state='HUMAN_REVIEW',
    control_mode='HUMAN_REQUIRED',
    pricing_readiness=least(coalesce(c.pricing_readiness,0.49),0.49),
    metadata=coalesce(c.metadata,'{}'::jsonb) || jsonb_build_object(
      'readinessReason',r.reason_code,
      'deterministicRiskEscalatedAt',now()
    ),
    updated_at=now()
from risk_cases r
...
where exists (
  ...
  where c.conversation_id=conv.id
    ...
    and c.metadata ? 'deterministicRiskEscalatedAt'
);
