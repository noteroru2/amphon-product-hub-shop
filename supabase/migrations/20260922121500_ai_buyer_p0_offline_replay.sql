-- P0 Regression + Offline Replay foundation.
-- This replay uses only stored structured evidence in Supabase. It never calls OpenAI.

create table if not exists public.ai_buyer_offline_replay_runs (
  id uuid primary key default gen_random_uuid(),
  policy_version text not null,
  status text not null default 'RUNNING'
    check (status in ('RUNNING','SUCCEEDED','FAILED')),
  total_cases integer not null default 0,
  price_now_cases integer not null default 0,
  needs_info_cases integer not null default 0,
...
create table if not exists public.ai_buyer_offline_replay_results (
  id uuid primary key default gen_random_uuid(),
  run_id uuid not null references public.ai_buyer_offline_replay_runs(id) on delete cascade,
  ...
  bucket text not null
    check (bucket in ('A_PRICE_NOW','B_NEEDS_INFO','C_HUMAN_REVIEW')),
  reason_code text not null,
  pricing_route text not null,
  identity_confidence numeric(6,4) not null default 0,
  spec_completeness numeric(6,4) not null default 0,
  condition_completeness numeric(6,4) not null default 0,
  model_name text,
  model_code text,
  missing_fields jsonb not null default '[]'::jsonb,
  requested_inputs jsonb not null default '[]'::jsonb,
  flags jsonb not null default '[]'::jsonb,
  pricing_tags jsonb not null default '[]'::jsonb,
  detected_battery_bad boolean not null default false,
  existing_battery_bad_tag boolean not null default false,
  battery_adjustment_applied boolean not null default false,
  latest_pricing_review_reason text,
  regression_codes jsonb not null default '[]'::jsonb,
  replayed_at timestamptz not null default now(),
  unique (run_id, case_id)
);

create index if not exists ai_buyer_offline_replay_results_run_idx
  on public.ai_buyer_offline_replay_results(run_id, bucket, reason_code);
create index if not exists ai_buyer_offline_replay_results_case_idx
  on public.ai_buyer_offline_replay_results(case_id, replayed_at desc);

alter table public.ai_buyer_offline_replay_runs enable row level security;
alter table public.ai_buyer_offline_replay_results enable row level security;

revoke all on table
  public.ai_buyer_offline_replay_runs,
  public.ai_buyer_offline_replay_results
